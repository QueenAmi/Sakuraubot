...
async def font_message(client, message):
    if message.reply_to_message:
        if message.reply_to_message.text:
            ...
            return await message.reply(emoji("gagal") + "<blockquote><b>ᴅɪ ʀᴇᴘʟʏ ɴɢᴇɴᴛᴏᴛ ᴘᴇsᴀɴɴʏᴀ ʙɪᴀʀ ʙɪsᴀ ᴘᴀᴋᴇ ғᴏɴᴛ.<b></blockquote>")
    else:
            ...
            (
                InlineQueryResultArticle(
                    ...
                    input_message_content=InputTextMessageContent(
                        "<blockquote><b>👇 sɪʟᴀʜᴋᴀɴ ᴘɪʟɪʜ sᴀʟᴀʜ sᴀᴛᴜ ғᴏɴᴛ ᴅɪʙᴀᴡᴀʜ.<b></blockquote>"
                    ),
                )
            )
        ],
...
async def font_next(client, callback_query):
    try:
        ...
        return await callback_query.answer(f"<blockquote><b>❌ Error: {error}<b></blockquote>", True)
