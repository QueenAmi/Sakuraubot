...
async def clearall(client, message):
    ...
    if len(message.command) == 1 and rep:
        ...
        try:
            ...
            pass
        await message.reply(f"berhasil mengahapus semua chat")
    else:
        if message.command[1].strip().lower() == "all":
            ...
            for kelot in biji:
                try:
                    ...
                    info = await client.resolve_peer(kelot)
                    await client.invoke(DeleteHistory(peer=info, max_id=0, revoke=True))
            await message.reply(f"sukses menghapus seluruh chat kamu")
            ...
            for kelot in bijo:
                try:
                    ...
                    info = await client.resolve_peer(kelot)
                    await client.invoke(DeleteHistory(peer=info, max_id=0, revoke=True))
            await message.reply(f"berhasil menghapus semua chat")
        else:
            ...
            try:
                ...
                info = await client.resolve_peer(who)
                await client.invoke(DeleteHistory(peer=info, max_id=0, revoke=True))
            await message.reply(f"gagal menghapus chat private kamu")
